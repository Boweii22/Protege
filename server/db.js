...
export async function completeGeneration({id,model,output,usage,routing,durationMs}){const sql=getSql();await sql`UPDATE ai_generations SET model=${model},output=${JSON.stringify(output)}::jsonb,token_usage=${JSON.stringify(usage||{})}::jsonb,routing=${JSON.stringify(routing||{})}::jsonb,duration_ms=${durationMs},status='complete',completed_at=NOW() WHERE id=${id}`}
