...
const protectionTables=await sql`SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_name IN ('rate_limit_windows','generation_leases','error_events') ORDER BY table_name`;
console.log(JSON.stringify({tables:tables.map(row=>row.table_name),indexes:indexes.map(row=>row.indexname),routingColumns,protectionTables:protectionTables.map(row=>row.table_name)}));
if(tables.length!==3||indexes.length<6||routingColumns.length!==2||protectionTables.length!==3)process.exit(1);
